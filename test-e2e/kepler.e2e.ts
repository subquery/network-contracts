...
async function planManagerTest() {
    ...
    const nextPlanId = await sdk.planManager.nextPlanId(INDEXER_ADDR);
    let plan = await sdk.planManager.plans(INDEXER_ADDR, nextPlanId);
    console.log(`plan created with index ${nextPlanId}: `);
    ...
    await sdk.planManager
        ...
        .acceptPlan(INDEXER_ADDR, cidToBytes32(DEPLOYMENT_ID), nextPlanId, overrides);
    ...
    await sdk.planManager.connect(indexer_wallet).removePlan(nextPlanId);
    plan = await sdk.planManager.plans(INDEXER_ADDR, nextPlanId);
    if (!plan.active) {
        console.log(`plan ${nextPlanId} has been successfully removed`);
    } else {
        console.log(`plan ${nextPlanId} remove failed`);
    }
}
...
async function purchaseOfferTest() {
    ...
    console.log('cancelled: ' + offer.active);
