...
contract Airdropper is Ownable {
    ...
    event RoundSettled(uint256 indexed roundId, address settleDestination);

    function setSettleDestination(address _settleDestination) external onlyOwner {
        settleDestination = _settleDestination;
    }
    
    function createRound(address _tokenAddr, uint256 _roundStratTime, uint256 _roundDeadline) external onlyOwner returns (uint256) {
        ...
        nextRoundId += 1;
        emit RoundCreated(nextRoundId - 1);
    ...
    function batchAirdrop(address[] calldata _addr, uint256[] calldata _roundId, uint256[] calldata _amount) external onlyOwner {
    ...
    function claimAirdrop(uint256 _roundId) public {
        require(roundRecord[_roundId].roundDeadline > block.timestamp && roundRecord[_roundId].roundStartTime < block.timestamp, 'invaild round to claim');
        uint256 amount = airdropRecord[msg.sender][_roundId];
        require(amount != 0, 'nothing claim');

        require(IERC20(roundRecord[_roundId].tokenAddress).balanceOf(address(this)) >= amount, 'Airdropper: insufficient assets');
    ...
    function settleEndedRound(uint256 _roundId) public {
        require(roundRecord[_roundId].roundDeadline < block.timestamp && roundRecord[_roundId].roundDeadline != 0, 'invaild round to settle');
        require(roundRecord[_roundId].unclaimedAmount != 0, 'none token left');
        require(IERC20(roundRecord[_roundId].tokenAddress).balanceOf(address(this)) >= roundRecord[_roundId].unclaimedAmount, 'Airdropper: insufficient assets');
        IERC20(roundRecord[_roundId].tokenAddress).safeTransfer(settleDestination, roundRecord[_roundId].unclaimedAmount);
        roundRecord[_roundId].unclaimedAmount = 0;
        emit RoundSettled(_roundId, settleDestination);
    }
