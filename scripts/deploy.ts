...
const main = async () => {
    try {
        ...
        let result;
        if (target === 'root') {
            result = await deployRootContracts(wallet.connect(rootProvider), config.contracts, {
                network: name,
                confirms,
                history,
            });
            ...
            result = await deployContracts(wallet.connect(childProvider), config.contracts, {
                network: name,
                confirms,
                history,
            });
