...
const main = async () => {
    const { name, wallet, childProvider, rootProvider, target, confirms, checkOnly, implementationOnly } =
        await setup();
    const filePath = `${__dirname}/../publish/${name}.json`;
    const deployment = JSON.parse(fs.readFileSync(filePath, { encoding: 'utf8' }));
    const matcher = yargs(hideBin(process.argv)).command('$0 [matcher]', '', (opt) =>
        opt.positional('matcher', {
            type: 'string',
        })
    ).argv.matcher;
