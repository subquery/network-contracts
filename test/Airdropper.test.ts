...
describe('Airdropper Contract', () => {
    ...
    describe('round test', () => {
        it('create round should work',async () => {
            await airdropper.createRound(sqtAddress, await lastestTime(mockProvider)+60*60, await futureTimestamp(mockProvider, 60*60*24*3));
        ...
        it('create round with invaild parameter should fail',async () => {
            await expect(airdropper.createRound(ZERO_ADDRESS, await lastestTime(mockProvider)+60*60, await futureTimestamp(mockProvider, 60*60*24*3))).to.be.revertedWith('invaild token address');
            await expect(airdropper.createRound(sqtAddress, 100, await futureTimestamp(mockProvider, 60*60*24*3))).to.be.revertedWith('invaild round time set');
            await expect(airdropper.createRound(sqtAddress, await lastestTime(mockProvider)+60*60*2, await lastestTime(mockProvider)+60*60)).to.be.revertedWith('invaild round time set');
        });
    });
    ...
    describe('airdrop test', () => {
        beforeEach(async () => {
            //roundId 0: expire 
            await airdropper.createRound(sqtAddress, await futureTimestamp(mockProvider, 60*60*2), await futureTimestamp(mockProvider, 60*60*24*2));
            //roundId 1: ongoing 
            await airdropper.createRound(sqtAddress, await futureTimestamp(mockProvider, 60*60*3), await futureTimestamp(mockProvider, 60*60*24*5));

            await token.increaseAllowance(airdropper.address, etherParse("100"));
        });
        it('batch airdrop should work',async () => {
            await airdropper.connect(wallet_0).batchAirdrop([wallet_1.address, wallet_2.address], [1, 1], [etherParse("10"), etherParse("20")]);
        ...
        it('duplicate airdrop should fail',async () => {
            ...
            await expect(airdropper.batchAirdrop([wallet_1.address], [1], [etherParse("20")])).to.be.revertedWith('duplicate airdrop');
        });
        it('airdrop invaild round should fail',async () => {
            await timeTravel(mockProvider, 60*60*2);
            await expect(airdropper.batchAirdrop([wallet_1.address], [0], [etherParse("10")])).to.be.revertedWith('invaild round to airdrop');
        });
        it('airdrop 0 amount should fail',async () => {
            await expect(airdropper.batchAirdrop([wallet_1.address], [1], [etherParse("0")])).to.be.revertedWith('invaild airdrop amount');
        });
    ...
    describe('claim airdrop test', () => {
        beforeEach(async () => {
            ...
            await airdropper.createRound(sqtAddress, await futureTimestamp(mockProvider, 60*60*2), await futureTimestamp(mockProvider, 60*60*24*2));
            await airdropper.batchAirdrop([wallet_1.address], [0], [etherParse("10")]);
            await airdropper.batchAirdrop([wallet_2.address], [0], [etherParse("20")]);
        });
        it('claim airdrop should work',async () => {
            ...
            expect(await (await airdropper.roundRecord(0)).unclaimedAmount).to.be.eq(etherParse("20"));
