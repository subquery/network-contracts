...
describe('Vesting Contract', () => {
    ...
    const checkAllocation = async (planId: number, user: string, allocation: number) => {
        expect((await vestingContract.userAllocation(user))[0]).to.equal(planId);
        expect((await vestingContract.userAllocation(user))[1]).to.equal(parseEther(allocation));
    ...
    describe('Allocate Vesting', () => {
        ...
        it('batch allocate vesting should work', async () => {
            ...
            console.log('==== gasused:', gasUsed); // 184338, 130918, 64979, 100579, 184054, 161712, 161660, // 161660/117430 108629/86514 106832/158064
